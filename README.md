...
SMR
  ...
  so on this one, i used SELECT capital FROM countries WHERE code='BRA'; and i got 211? which didn't really make sense. And then i noticed code 2 was BR so I thought maybe that was a column in the cities table but it wasn't.
 the only number in the citites table was the id column, and so I looked up the id of 211, Brasillia, which happes to be the capital of Brazil. But it's not explicit that capital 211 correlates to an id. V silly. 
